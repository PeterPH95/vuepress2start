...
### 6.setup的两个[注意点](https://www.bilibili.com/video/BV1Zy4y1K7SH?p=149&vd_source=3222fe640ca9017c65847877e892e557)
...
- 两个小“坑”：
  ...
  - 监视reactive定义的响应式数据中某个属性（为对象类型）时：deep配置有效。
...
<div style="border:1px solid black;width:48%; text-align:center">
    <strong>vue2.x的生命周期</strong>
    <img src="https://v2.cn.vuejs.org/images/lifecycle.png" alt="lifecycle_vue2" />
</div>
<div style="border:1px solid black;width:48%; text-align:center">
    <strong>vue3.0的生命周期</strong>
    <img src="https://cn.vuejs.org/assets/lifecycle.16e4c08e.png" alt="lifecycle_vue3" />
</div>
</div>
...
- 应用: 要将响应式对象中的某个属性单独提供给外部使用，并且让解构出的属性保持响应式时。
...
- readonly: 让一个响应式数据变为只读的（深只读）(<strong style="color:red">所有的数据都不可改</strong>)。
- shallowReadonly：让一个响应式数据变为只读的（浅只读）（<strong style="color:red">最外层不能改，深层的数据可以改</strong>）。
- 应用场景: 不希望数据被修改时。
...
- 我们可以更加优雅的组织我们的代码，函数。让相关功能的代码更加有序的组织在一起。<strong>通过自定义的`hook`函数，将业务的数据和函数封装到一起。</strong>
